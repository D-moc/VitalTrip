...
const CaptainLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await login(form, "captain"); // instantly update Navbar + role
      navigate("/"); // go to home, not dashboard
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#004e92] to-[#000428] text-white">
      <AuthCard title="Captain Login ⚓" subtitle="Authorized personnel only">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <motion.button
            whileTap={{ scale: 0.95 }}
            className="w-full py-3 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg text-white font-semibold shadow-lg hover:shadow-cyan-400/40 transition-all"
            disabled={loading}
          >
